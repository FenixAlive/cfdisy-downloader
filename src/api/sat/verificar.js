import { SATService } from '../lib/sat-service.js';

export async function POST({ request }) {
    try {
        const { id, cerBase64, keyBase64, password } = await request.json();

        if (!id || !cerBase64 || !keyBase64 || !password) {
            return Response.json({ success: false, message: 'Faltan campos requeridos' }, { status: 400 });
        }

        // Convertir base64 a buffers
        const cerBuffer = Buffer.from(cerBase64.split(',')[1], 'base64');
        const keyBuffer = Buffer.from(keyBase64.split(',')[1], 'base64');

        // Parsear credenciales
        const credentials = await SATService.parseCredentials(cerBuffer, keyBuffer, password);

        // Verificar solicitud
        const result = await SATService.verificarSolicitud(credentials, credentials.token, id);

        return Response.json({
            success: true,
            status: result.estado,
            message: result.mensaje || 'Verificación completada',
            packageId: result.paqueteId
        });

    } catch (error) {
        console.error('Error en /api/sat/verificar:', error);
        return Response.json({
            success: false,
            message: error.message || 'Error interno del servidor'
        }, { status: 500 });
    }
}
